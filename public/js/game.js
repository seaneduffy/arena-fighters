...
var React = require('react'),
    _handleStartTwoPlayerGame2 = null,
...
function movePlayer(player, angle, amount) {
	if (!player.dead) player.walk(amount, angle);
}
...
var imageInfo = fireBtnJson['fire_up'].frame;
...
Object.defineProperties(GameObject.prototype, {
	...
	'spriteMeta': {
		set: function set(spriteMeta) {
			...
			if (!!this.spriteLabels) this.initSprites();
		},
		get: function get() {
...
Object.defineProperties(Sprite.prototype, {
	...
	'draw': {
		value: function value() {
			...
			this.context.drawImage(this.sheet, frameMeta.x, frameMeta.y, frameMeta.w, frameMeta.h, 0, 0, this.width, this.height);
		}
	},
	'addUpdate': {
		value: addUpdate
	...
	'cycleStart': {
		get: function get() {
			if (this._cycleStart === 'undefined') return this._cycleStart = 0;
...
function updateDisplay(label) {
	var imageInfo = spriteJson[label].frame;
...
function _init() {
	...
	var imageInfo = spriteJson['center'].frame;
...
function pressMove(e) {
	var x = e.pageX,
	    ...
	    deltaY = y - centerY;
	if (Math.abs(deltaX) > sensitivity || Math.abs(deltaY) > sensitivity) {
		var angle = geom.getAngle(centerX, centerY, x, y);
		callback(angle);
	} else {
		callback(-1, 0);
	}
...
function callback(angle, amount) {
	...
	if (angle === -1) {
		updateDisplay('center');
	} else if (angle <= pi / 8 || angle > 15 * pi / 8) updateDisplay('up');else if (angle <= 3 * pi / 8) updateDisplay('upright');else if (angle <= 5 * pi / 8) updateDisplay('right');else if (angle <= 7 * pi / 8) updateDisplay('downright');else if (angle <= 9 * pi / 8) updateDisplay('down');else if (angle <= 11 * pi / 8) updateDisplay('downleft');else if (angle <= 13 * pi / 8) updateDisplay('left');else if (angle <= 15 * pi / 8) updateDisplay('upleft');
	for (var i = 0; i < l; i++) {
